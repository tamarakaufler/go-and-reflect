...
func RunMarshalling() {
	log.Println("========================= custom marshal2 ============================")
	u2 := User2{
		Name: "Tamara",
		Age:  45,
		Note: []rune("Programmer"),
		NI:   []int32{9, 8, 7, 6, 5, 4, 3, 2, 1},
		Address: Address2{
			Street:   []rune("101 Tyttenhanger"),
			City:     []rune("St Albans"),
			Postcode: "AL4",
			LatLng: LatLng{
				Lat: 40.0000,
	...
	log.Println("========================= default marshal2 ============================")
	u3 := User3{
		Name: "Lucien",
		Age:  35,
		Note: []rune("Jazz guitarist, cool guy"),
		NI:   []int32{9, 8, 7, 6, 5, 4, 3, 2, 1},
		Address: Address2{
			Street:   []rune("2B Matheson Road"),
			City:     []rune("London"),
			Postcode: "W14",
		},
	}
...
func (a Address2) MarshalJSON() ([]byte, error) {
	dst := map[string]interface{}{}
	dst, err := marshalMe(a, dst)
	if err != nil {
		return nil, err
	}
	return json.Marshal(&dst)
...
func (a *Address2) UnmarshalJSON(b []byte) error {
	log.Printf("\tbytes to unmarshal Address2: %s\n", string(b))

	return unmarshalMe(b, a)
}
...
func unmarshalMe(b []byte, dst interface{}) error {
	...
	msCfg := &mapstructure.DecoderConfig{
		DecodeHook: mapstructure.ComposeDecodeHookFunc(
			DecodeStringToInt32SliceHookFunc(),
		),
...
func DecodeStringToInt32SliceHookFunc() mapstructure.DecodeHookFunc {
	return func(
		f reflect.Type,
		t reflect.Type,
		...
		//log.Printf("\t??? f=%+v, t=%+v\n", f, t)
		//log.Printf("\t\t??? KIND f=%+v, t=%+v\n", f.Kind(), t.Kind())
		//log.Printf("\t\t??? NAME f=%+v, t=%+v\n", f.Name(), t.Name())

		_, ok := customFieldsM[t.Name()]
